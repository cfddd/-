> **学习资料**：
> 1. https://liaoxuefeng.com/books/jerrymouse/load-webapp/index.html
## 加载webAPP
一个正常的Web服务器是从外部加载这些组件的，根据Servlet规范，Web App开发者完成了`Servlet`、`Filter`和`Listener`等组件后，需要按规范把它们打包成`.war`文件。`.war`文件本质上就是一个jar包，但它的目录组织如下：
